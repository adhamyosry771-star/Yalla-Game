...
export const VoiceRoom: React.FC<VoiceRoomProps> = ({ 
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "design"), (docSnap) => {
    ...
    <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col overflow-hidden w-full h-full animate-in fade-in duration-300" dir="rtl">
          ...
          {micStates.map((mic, i) => (
            <div key={i} className="flex flex-col items-center gap-2">
              <button 
                ...
                className="w-[52px] h-[52px] sm:w-[62px] sm:h-[62px] flex items-center justify-center relative transition-all duration-300 active:scale-90"
              >
                {mic.user ? (
                  <div className="w-full h-full relative flex items-center justify-center animate-in zoom-in duration-200">
                    {designSettings?.micOccupiedBg && (
                       <img src={designSettings.micOccupiedBg} className="absolute inset-0 w-full h-full object-contain z-0" alt="" />
                    )}
                    <img 
                      src={mic.user.photoURL || "https://picsum.photos/100"} 
                      className="w-full h-full rounded-full object-cover relative z-10 border border-white/20 shadow-lg" 
                      alt={mic.user.displayName}
                    />
                    {mic.user.uid === user?.uid && !isMicMuted && (
                      <div className="absolute inset-0 z-20 pointer-events-none">
                         <div className="w-full h-full rounded-full border-[3px] border-purple-400 animate-ping opacity-40"></div>
                      </div>
                    )}
                  </div>
                ) : (
        ...
        <div className="px-3 pb-6 flex items-center gap-2 mt-auto">
          {userIsOnMic ? (
            <button 
              ...
              className="w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md border border-white/20 transition-all bg-white/10 text-white shadow-lg active:scale-90 overflow-hidden"
            >
          ...
          {/* زر القائمة - شكلي فقط حالياً */}
          <button 
            className="w-10 h-10 rounded-full bg-white/5 backdrop-blur-md border border-white/10 flex items-center justify-center active:scale-90 transition-transform relative group"
          >
            <div className="grid grid-cols-2 gap-[3px] p-2">
              <div className="w-[6px] h-[6px] rounded-[1px] bg-white opacity-80 group-hover:opacity-100 transition-opacity"></div>
              <div className="w-[6px] h-[6px] rounded-[1px] bg-white opacity-80 group-hover:opacity-100 transition-opacity"></div>
              <div className="w-[6px] h-[6px] rounded-[1px] bg-white opacity-80 group-hover:opacity-100 transition-opacity"></div>
              <div className="w-[6px] h-[6px] rounded-[1px] bg-white opacity-80 group-hover:opacity-100 transition-opacity"></div>
            </div>
              ...
              <div className="flex items-center -space-x-2 overflow-visible">
                {usersOnMics.length > 0 ? (
                  usersOnMics.slice(0, 5).map((u, i) => (
                    <button key={u.uid || i} onClick={() => toggleUserSelection(u.uid)} className="w-9 h-9 rounded-full relative flex items-center justify-center bg-purple-900 shadow-md transition-all active:scale-90">
                      {selectedUserIds.has(u.uid) && <div className="absolute inset-[-2px] rounded-full border-[3px] border-[#1a0b2e]/60 pointer-events-none z-0"></div>}
              ...
              <div className="flex items-center h-9 w-[131px] relative overflow-visible">
                 <div className="flex items-center h-full w-full rounded-full border border-[#2d1252]/60 overflow-hidden shadow-lg">
                    <div className="basis-1/2 h-full relative bg-[#2d1252]/30">
                       <button onClick={(e) => { e.stopPropagation(); setShowQuantityMenu(!showQuantityMenu); }} className="w-full h-full flex items-center justify-center gap-1 transition-all active:scale-95"><i className={`fas fa-chevron-up text-[7px] text-white/60 ${showQuantityMenu ? 'rotate-180' : ''}`}></i><span className="text-[9.5px] font-black text-white/90">x{selectedQuantity}</span></button>
                       {showQuantityMenu && (
                         <div className="absolute bottom-[calc(100%+12px)] left-0 w-[131px] bg-[#1a0b2e]/60 backdrop-blur-2xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-[150] animate-in zoom-in duration-200 origin-bottom">
                    ...
                    <button className="basis-1/2 h-full bg-[#2d1252]/85 text-white text-[9.5px] font-black active:bg-[#3d1a6e] border-r border-[#2d1252]/60">إرسال</button>
                 </div>
              </div>
            </div>
          </div>
      ...
      {showParticipants && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-6 animate-in fade-in" onClick={() => setShowParticipants(false)}>
          <div className="w-full max-w-[300px] bg-[#1a0b2e]/85 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-5 border-b border-white/5 flex justify-between items-center">
              <h3 className="text-white font-black text-sm">المتواجدون ({allPresentUsers.length})</h3>
              <button onClick={() => setShowParticipants(false)} className="text-white/40"><i className="fas fa-times text-xs"></i></button>
            </div>
            <div className="max-h-[380px] overflow-y-auto p-3 space-y-2 scrollbar-hide">
              {allPresentUsers.map((u, idx) => (
                <div key={u.uid || idx} className="flex items-center gap-3 p-2 rounded-2xl bg-white/5 border border-white/5">
                  <img src={u.photoURL || "https://picsum.photos/100"} className="w-11 h-11 rounded-full object-cover border border-white/10" />
      ...
      {showMicActions && selectedMicIndex !== null && (
        <div className="fixed inset-0 z-[250] flex items-end justify-center bg-black/60 backdrop-blur-[2px] animate-in fade-in" onClick={() => setShowMicActions(false)}>
          <div className="w-full max-w-md bg-[#1a0b2e]/60 backdrop-blur-2xl rounded-t-[2.5rem] p-6 pb-10 space-y-4 animate-slide-up border-t border-white/10" onClick={e => e.stopPropagation()}>
            <div className="flex flex-col items-center mb-4"><div className="w-12 h-1.5 bg-white/20 rounded-full mb-6"></div><h3 className="text-white font-black text-lg">تحكم المايك {selectedMicIndex + 1}</h3></div>
            <div className="grid grid-cols-1 gap-3">
              {micStates[selectedMicIndex].user?.uid === user?.uid ? (
                <button onClick={() => leaveMic(selectedMicIndex)} className="w-full py-4 bg-red-500/20 text-red-400 rounded-2xl font-black flex items-center justify-center gap-3 border border-red-500/20 active:scale-95 transition-all"><i className="fas fa-sign-out-alt"></i> مغادرة المايك</button>
              ) : (
                <button onClick={() => takeMic(selectedMicIndex)} className="w-full py-4 bg-purple-600 text-white rounded-2xl font-black flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-all"><i className="fas fa-hand-pointer"></i> أخذ المايك</button>
              )}
              {isRoomOwner && micStates[selectedMicIndex].user?.uid !== user?.uid && (
                <button onClick={() => toggleLockMic(selectedMicIndex)} className={`w-full py-4 rounded-2xl font-black flex items-center justify-center gap-3 border transition-all active:scale-95 ${micStates[selectedMicIndex].status === 'locked' ? 'bg-white/10 text-white border-white/20' : 'bg-white/5 text-white/80 border-white/10'}`}><i className={`fas ${micStates[selectedMicIndex].status === 'locked' ? 'fa-lock-open' : 'fa-lock'}`}></i>{micStates[selectedMicIndex].status === 'locked' ? 'فتح المايك' : 'قفل المايك'}</button>
              )}
